...
class SyncStatus {
  ...
  SyncStatus copyWith({SyncPhase? phase, DateTime? lastPullAt, DateTime? lastPushAt, AppError? lastError, bool? online, bool clearError = false}) => SyncStatus(
...
class SyncEngine {
  ...
  /// Dipanggil setelah push dengan object (objectType, objectId) yang mutasinya diterima/konflik oleh server —
  /// app memuat ulang detail & checklist run dari server supaya `allowed_actions`/status lokal segar
  /// tanpa menunggu pull bundle berikutnya. Kegagalan (offline) diabaikan.
  final Future<void> Function(Set<({String objectType, String objectId})> objects)? onApplied;
  ...
  Future<void> push({bool force = false}) async {
    // Mutasi `sending` saat push dimulai = sisa proses yang terbunuh di tengah request (app dibunuh OS,
    // isolate workmanager). Single-flight menjamin tidak ada push lain di isolate ini; server idempoten
    // per client_mutation_id (C6) sehingga aman dikirim ulang. Tanpa ini mutasi macet selamanya & memblokir
    // mutasi berikutnya untuk object yang sama (C9).
    await (db.update(db.mutations)..where((t) => t.status.equals('sending'))).write(const MutationsCompanion(status: Value('pending')));
    final applied = <({String objectType, String objectId})>{};
    try {
      await _pushLoop(force: force, applied: applied);
    } finally {
      if (applied.isNotEmpty && onApplied != null) {
        try {
          await onApplied!(applied);
        } catch (_) {
          // offline / server error: pull berikutnya yang menyegarkan
        }
      }
    }
  }

  Future<void> _pushLoop({required bool force, required Set<({String objectType, String objectId})> applied}) async {
    while (true) {
      final now = _now();
      final rows =
          await (db.select(db.mutations)
      ...
      try {
        ...
        await db.transaction(() async {
          for (final r in ordered) {
            ...
            await (db.update(db.mutations)..where((t) => t.clientMutationId.equals(r.clientMutationId))).write(
              MutationsCompanion(status: const Value('pending'), attempts: Value(attempts), nextAttemptAt: Value(now.add(_backoff(attempts))), lastError: Value(e.message)),
            );
          }
        });
        rethrow;
      ...
      await db.transaction(() async {
        ...
        for (final res in out.results) {
          ...
          if (res.isApplied || res.isConflict) applied.add((objectType: row.objectType, objectId: row.objectId));
          // Transisi diterima → status server langsung diterapkan ke baris lokal (respons `{status}`),
          // supaya UI tidak mundur ke status lama saat mutasi keluar dari overlay pending.
          final st = res.response?['status'];
          if (res.isApplied && WorkAction.transitions.contains(row.action) && st is String) {
            await repo.applyServerStatus(row.objectId, st);
          }
        }
        ...
        for (final r in ordered.where((r) => !seen.contains(r.clientMutationId))) {
          await (db.update(db.mutations)..where((t) => t.clientMutationId.equals(r.clientMutationId))).write(const MutationsCompanion(status: Value('pending')));
  ...
  Future<void> _applyResult(MutationRow row, MutationResult res, DateTime serverTime) async {
    ...
    if (res.isApplied) {
      await upd.write(MutationsCompanion(status: const Value('synced'), response: Value(res.response), reasonCode: const Value(null), lastError: const Value(null), syncedAt: Value(_now())));
    ...
    if (res.isConflict) {
      ...
      await upd.write(
        MutationsCompanion(status: const Value('conflict'), reasonCode: Value(res.reasonCode), lastError: Value(_conflictMessage(res)), response: Value(res.response), syncedAt: Value(_now())),
      );
      if (row.action == WorkAction.attachPhoto) await _bindUpload(row, res.response, accepted: true);
      return;
    }
    ...
    if (res.isSeqGap) {
      ...
      await (db.update(db.mutations)..where((t) => t.objectId.equals(row.objectId) & t.seq.isSmallerThanValue(row.seq) & t.status.equals('failed'))).write(
        const MutationsCompanion(status: Value('pending'), nextAttemptAt: Value(null)),
      );
      await upd.write(
        MutationsCompanion(
  ...
  static String _conflictMessage(MutationResult res) => switch (res.reasonCode) {
    'INVALID_TRANSITION' =>
      res.detail == 'EVIDENCE_REQUIRED' ? 'Foto wajib belum lengkap — lengkapi evidence lalu selesaikan lagi.' : 'Status di server sudah berubah; aksi tidak diterapkan. Evidence tetap tersimpan.',
  ...
  Future<void> _bindUpload(MutationRow row, Map<String, dynamic>? response, {required bool accepted}) async {
    ...
    if (accepted && uploadUrl != null && attachmentId != null) {
      await upd.write(PendingFilesCompanion(attachmentId: Value(attachmentId), uploadUrl: Value(uploadUrl), uploadState: const Value('ready'), nextAttemptAt: const Value(null)));
    } else {
  ...
  Future<void> uploadPhotos() async {
    final now = _now();
    final files =
        await (db.select(db.pendingFiles)
