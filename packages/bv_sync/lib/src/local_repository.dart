...
class LocalWorkRepository {
  ...
  Future<void> applyBundle(SyncBundle b) => db.transaction(() async {
        ...
        for (final pb in b.patrolTasks) {
          ...
          for (final cp in pb.checkpoints) {
            await db.into(db.patrolCheckpoints).insertOnConflictUpdate(PatrolCheckpointsCompanion.insert(
              id: cp.id,
              taskId: pb.task.id,
        ...
        for (final r in b.checklistRuns) {
          await db.into(db.checklistRuns).insertOnConflictUpdate(ChecklistRunsCompanion.insert(
            id: r.id,
            objectType: r.objectType,
            objectId: r.objectId,
            json: r.toJson(),
            updatedAt: b.serverTime,
          ));
        }
        for (final l in b.locations) {
          await db.into(db.locations).insertOnConflictUpdate(LocationsCompanion.insert(
            id: l.id,
            name: l.name,
            locationType: Value(l.locationType),
            pathText: Value(l.pathText),
            qrCode: Value(l.qrCode),
          ));
        }
        ...
        for (final id in {
          ...b.tasks.map((e) => e.id),
          ...b.workOrders.map((e) => e.id),
          ...b.cleaningTasks.map((e) => e.id),
          ...b.patrolTasks.map((e) => e.task.id),
        }) {
  ...
  Future<void> _upsertWorkItem(WorkItem w, {String? kind}) async {
    final k = kind ??
        (w.objectType == ObjectType.workOrder
            ? 'work_order'
            : switch (w.type) { 'patrol' => 'patrol', 'cleaning' => 'cleaning', 'inspection' => 'inspection', _ => 'task' });
  ...
  WorkItem _rowToItem(WorkItemRow r, List<Mutation> pending) =>
      LocalOverlay.applyToWorkItem(WorkItem.fromJson(r.json), pending.where((m) => m.objectId == r.id));
  ...
  Stream<List<WorkItem>> watchWorkItems({Set<String>? kinds, String? assigneeUserId, bool openOnly = false}) {
    final q = db.select(db.workItems)
      ..orderBy([
  ...
  Future<String?> syncStateOf(String id) async =>
      (await (db.select(db.workItems)..where((t) => t.id.equals(id))).getSingleOrNull())?.syncState;
  ...
  Future<void> saveRun(ChecklistRun run) => db.into(db.checklistRuns).insertOnConflictUpdate(ChecklistRunsCompanion.insert(
        id: run.id,
  ...
  Stream<List<CheckpointScan>> watchCheckpoints(String taskId) {
    final rows$ = (db.select(db.patrolCheckpoints)
          ..where((t) => t.taskId.equals(taskId))
          ..orderBy([(t) => OrderingTerm.asc(t.sortOrder)]))
        .watch();
    final pending$ = _pendingMutationsQuery(objectId: taskId).watch();
    return _combine2(rows$, pending$,
        (rows, pending) => LocalOverlay.applyToCheckpoints(rows.map((r) => CheckpointScan.fromJson(r.json)).toList(), pending));
  }

  Future<CheckpointScan?> checkpointByQr(String taskId, String qrCode) async {
    final row = await (db.select(db.patrolCheckpoints)..where((t) => t.taskId.equals(taskId) & t.qrCode.equals(qrCode)))
        .getSingleOrNull();
  ...
  Future<List<LocationLite>> locations({String? q}) async {
    ...
    return (await s.get())
        .map((r) => LocationLite(id: r.id, name: r.name, locationType: r.locationType, pathText: r.pathText, qrCode: r.qrCode))
        .toList();
  }
  ...
  Future<List<AssetLite>> assets({String? q}) async {
    ...
    return (await s.get())
        .map((r) => AssetLite(id: r.id, assetCode: r.assetCode, name: r.name, status: r.status, locationId: r.locationId, qrCode: r.qrCode))
        .toList();
  }
  ...
  Stream<List<MutationRow>> watchMutationRows(String objectId) => (db.select(db.mutations)
        ..where((t) => t.objectId.equals(objectId))
        ..orderBy([(t) => OrderingTerm.desc(t.seq)]))
      .watch();

  Stream<List<PendingFileRow>> watchPendingFiles(String objectId) =>
      (db.select(db.pendingFiles)..where((t) => t.objectId.equals(objectId) & t.uploadState.isNotIn(const ['confirmed']))).watch();
  ...
  Future<Mutation> enqueue(String objectType, String objectId, String action, Map<String, dynamic> payload,
      {String? clientMutationId}) async {
    final seq = await _nextSeq(objectId);
    final now = _now();
    final id = clientMutationId ?? _uuid.v7();
    await db.into(db.mutations).insert(MutationsCompanion.insert(
      clientMutationId: id,
  ...
  Future<Mutation> answerChecklist(ChecklistRun run, ChecklistRunItem item, AnswerInput input, {CapturedPhoto? photo}) =>
      db.transaction(() async {
        String? clientAttachmentId;
        if (photo != null) {
          clientAttachmentId = await _queuePhoto(run.objectType, run.objectId, photo,
              attachmentType: AttachmentType.checklist, checklistItemId: item.id);
        }
        final payload = {
          'item_id': item.id,
          'run_id': run.id,
          ...input.copyWith(clientAttachmentId: clientAttachmentId ?? input.clientAttachmentId, clientRecordedAt: _now()).toJson(),
        };
  ...
  Future<String> attachPhoto(String objectType, String objectId, CapturedPhoto photo,
          {String attachmentType = AttachmentType.photo}) =>
      db.transaction(() => _queuePhoto(objectType, objectId, photo, attachmentType: attachmentType));

  Future<String> _queuePhoto(String objectType, String objectId, CapturedPhoto photo,
      {required String attachmentType, String? checklistItemId}) async {
    ...
    await db.into(db.pendingFiles).insert(PendingFilesCompanion.insert(
  ...
  Future<Mutation> addComment(String objectType, String objectId, String body) => db.transaction(
      () => enqueue(objectType, objectId, WorkAction.addComment, {'body': body, 'client_comment_id': _uuid.v7()}));
  ...
  Future<Mutation> addFinding(String objectType, String objectId, CreateFindingInput input, {CapturedPhoto? photo}) =>
      db.transaction(() async {
  ...
  Future<Mutation> reportIncident(String taskId, CreateIncidentInput input) => db.transaction(
      () => enqueue(ObjectType.task, taskId, WorkAction.reportIncident, input.copyWith(clientRecordedAt: _now()).toJson()));
  ...
  Future<void> retryFailed(String objectId) => db.transaction(() async {
        await (db.update(db.mutations)..where((t) => t.objectId.equals(objectId) & t.status.equals('failed')))
            .write(const MutationsCompanion(status: Value('pending'), attempts: Value(0), nextAttemptAt: Value(null), lastError: Value(null)));
        await refreshSyncState(objectId);
      });
  ...
  Future<void> dismissConflict(String objectId) => db.transaction(() async {
        await (db.update(db.mutations)..where((t) => t.objectId.equals(objectId) & t.status.equals('conflict')))
            .write(const MutationsCompanion(status: Value('conflict_seen')));
        await refreshSyncState(objectId);
      });
  ...
  Future<void> refreshSyncState(String objectId) async {
    ...
    final files = await (db.select(db.pendingFiles)
          ..where((t) => t.objectId.equals(objectId) & t.uploadState.isNotIn(const ['confirmed', 'rejected'])))
        .get();
    String state;
    String? msg;
    ...
    await (db.update(db.workItems)..where((t) => t.id.equals(objectId)))
        .write(WorkItemsCompanion(syncState: Value(state), syncMessage: Value(msg)));
  }
  ...
  Future<void> prune({Duration olderThan = const Duration(days: 7)}) async {
    ...
    await (db.delete(db.mutations)
          ..where((t) => t.status.isIn(const ['synced', 'conflict_seen']) & t.syncedAt.isSmallerThanValue(cutoff)))
        .go();
...
Stream<R> _combine2<A, B, R>(Stream<A> a, Stream<B> b, R Function(A, B) f) {
  ...
  c = StreamController<R>(
    onListen: () {
      sa = a.listen((v) { va = v; ha = true; emit(); }, onError: c.addError);
      sb = b.listen((v) { vb = v; hb = true; emit(); }, onError: c.addError);
    },
    onCancel: () async {
      await sa?.cancel();
      await sb?.cancel();
